...
export const AppProvider = ({ children }: { children: ReactNode }) => {
  ...
  const updateMember = async (updatedMember: Member, silent: boolean = false) => {
    ...
    try {
      const res = await fetch(`http://<DIN_IP_ELLER_HOST>:3000/update-member/${updatedMember.id}`,{ // Her skal din private IP-adresse inn, har byttet den ut med tekst i henhold til personvern.
        method: 'PUT',
  ...
  const deleteMember = async (memberId: string) => {
    ...
    try {
      ...
      const res = await fetch(`http://<DIN_IP_ELLER_HOST>:3000/delete-member/${memberId}`,{
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const completeTask = async (memberId: string, taskId: string) => {
    try {
      ...
      const res = await  fetch(`http://<DIN_IP_ELLER_HOST>:3000/complete-task/${memberId}/${taskId}`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const fetchMembersForAdmin = async (adminId: string) => {
    try {
      const token = await authInstance.currentUser?.getIdToken(true);
      const res = await fetch(`http://<DIN_IP_ELLER_HOST>:3000/members`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
